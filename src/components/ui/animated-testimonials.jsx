...
export const AnimatedTestimonials = ({
  testimonials,
  autoplay = true, // Make autoplay true by default
}) => {
  const [active, setActive] = useState(0);
  ...
  useEffect(() => {
    if (autoplay) {
      const interval = setInterval(handleNext, 5000); // Automatically change every 5 seconds
      return () => clearInterval(interval); // Cleanup on component unmount or when autoplay is disabled
    }
  }, [autoplay]);
